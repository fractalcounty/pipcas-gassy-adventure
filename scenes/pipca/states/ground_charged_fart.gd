...
func fart():
	...
	#target.velocity = - target.ground_fart_speed * vector
	...
	if target.charge_meter == 0:
		...
		target.velocity.y = - target.ground_fart_speed * (target.charge_meter*050)
	
	#mouse_position = target.get_global_mouse_position()
